...
class VETToken extends Token {
  ...
  * */
  async createTransaction ({ address, amount }) {
    ...
    const dataToSend = `0x${
      TRANSFER_METHOD_SIGNATURE
    }${this.addLeadingZero(address.substring(TWO), ARGUMENT_SIZE)
    }${this.addLeadingZero(new this.BN(amount).toString(hexBase), ARGUMENT_SIZE)}`
  ...
  async availableBalance () {
    ...
    return available.lt(new this.BN(0))
      ? '0'
      : this.toCurrencyUnit(available)
  }
