import {
  ...
  Keypair,
  Asset,
  TransactionBuilder } from 'stellar-sdk'
...
class XlmExplorer extends Explorer {
  ...
  async getTransaction (selfAddress, txId) {
    ...
    try {
      ...
      const { records: [operation] } = await transaction.operations()
  ...
  async sendTransaction (txObject, senderAddress, privateKey, fee) {
    ...
    const txBuilder = new TransactionBuilder(srcAccount, { fee, networkPassphrase: Networks.PUBLIC })
  ...
  async getTransactions ({ address, offset = 0, limit = this.defaultTxLimit }) {
    try {
      ...
      for (let idx = 0; idx < transactions.records.length; idx += 1) {
        ...
        const txOperations = operations
          .records
          .filter((op) => op.transaction_hash === tx.id)
        ...
        for (let oCounter = 0; oCounter < txOperations.length; oCounter += 1) {
          ...
          if (operation.type === 'create_account') {
            list.push(
              this.parseCreateAccountOperation(currentLedgerVersion, tx, operation, address)
            )
          } else if (operation.type === 'payment') {
            list.push(
              this.parsePaymentOperation(currentLedgerVersion, tx, operation, address)
            )
          }
        }
      }
