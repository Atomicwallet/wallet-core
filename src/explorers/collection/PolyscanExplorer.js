...
class PolyscanExplorer extends Explorer {
  ...
  getTransactionsModifiedResponse (...args) {
    return new Transaction(
      super.getTransactionsModifiedResponse(...args)
    )
  }
  ...
  getTxDateTime (tx) {
    return new Date(
      Number(tx.timeStamp) * SECONDS_TO_MILLISECONDS
    )
  }
  ...
  async getTokenTransactions ({ address }) {
    ...
    return response.result.map((tx) => {
      return new Transaction({
        ...
        name: tx.tokenName,
        walletid: getTokenId({ ticker: tx.tokenSymbol, contract: tx.contractAddress, parent: this.wallet.parent }),
