...
class YoroExplorer extends Explorer {
  ...
  async getTransactionsQueued ({ blockHash, address, recursion = false, after = undefined }) {
    ...
    if (response.length > 0) {
      ...
      const resolved = await this.getTransactionsQueued({ blockHash: untilBlock, address, recursion: true, after: afterParams })
  ...
  getTxValue (selfAddress, tx) {
    ...
    const sentToMyself = inputsAddress.concat(outputsAddress)
      .every((address) => address === selfAddress)

    let value

    if (sentToMyself) {
      const outputsAmount = tx.outputs
        .reduce((prev, cur) => prev.add(new this.wallet.BN(cur.amount)), new this.wallet.BN(0))

      const inputsAmount = tx.inputs
        .reduce((prev, cur) => prev.add(new this.wallet.BN(cur.amount)), new this.wallet.BN(0))
