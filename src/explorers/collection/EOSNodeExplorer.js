...
class EOSNodeExplorer extends Explorer {
  ...
  modifyTransactionsResponse (response, address) {
    return super.modifyTransactionsResponse(response.actions.filter((transaction) => {
  ...
  async sendTransaction (rawtx, privateKey) {
    ...
    try {
      const response = await eos.transact(transaction, { blocksBehind: 10, expireSeconds: 3600 })
