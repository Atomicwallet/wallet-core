...
class BlockscoutExplorer extends Explorer {
  ...
  async request (url, method = 'get', params = {}, type = UNDEFINED_OPERATION_ERROR) {
    ...
    if (response.status === '0') {
      ...
      throw new ExplorerRequestError({ type, error: response.message, url: `${this.config.baseUrl}${url}`, instance: this })
    }
  ...
  modifyTokenTransactionsResponse (response, selfAddress) {
    return response.map((transfer) => {
      return new Transaction({
        walletid: getTokenId({ ticker: transfer.tokenSymbol, parent: this.wallet.ticker, contract: transfer.contractAddress }),
