...
class GuardaMoneroProxy extends Explorer {
  ...
  async getAccount (address) {
    return this.request(
      `account/${address}`, 'GET', undefined, undefined, {
        headers: {
  ...
  async postAccount (address) {
    return this.request(
      'account', 'POST', { address, API_key: this.config.apiKey }
    )
  }
  ...
  async txData (hash) {
    return this.request(
      `tx/data/${hash}`, 'GET', undefined, undefined, {
        headers: {
          API_key: this.config.apiKey,
        },
      }
  ...
  getTxValue (tx) {
    return Number(this.wallet.toCurrencyUnit(this.getTxDirection(tx) // TODO: clean after ref merge
      ? tx.total_received
  ...
  modifyTransactionsResponse (selfAddress, response, lastHeight) {
    ...
    return response.transactions
      .filter(this.wallet.filterTransactions)
  ...
  async request (...args) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!isRequesting) {
        super.request(...args).then((requestResult) => {
