...
class NearRPCExplorer extends Explorer {
  ...
  async sendTransaction (rawtx) {
    const { transaction } = await this.provider.sendJsonRpc('broadcast_tx_commit', [
      rawtx,
    ])
  ...
  async getInfo (selfAddress) {
    try {
      const { runtime_config: { storage_amount_per_byte: storageAmountPerByte } } = await this.getNodeConfig()
      ...
      const stateStaked = new this.wallet.BN(info.storage_usage)
        .mul(new this.wallet.BN(storageAmountPerByte))
  ...
  async fetchStakingInfo (selfAddress, { activeValidators = [] } = {}) {
    ...
    const [unstaking, staked, availableWithdrawals, pendingWithdrawals] = ['unstaking', 'staked', 'availableWithdrawals', 'pendingWithdrawals']
      .map((field) => Object.values(validators)
    ...
    return {
      ...
      availableWithdrawals: new Amount(new this.wallet.BN(availableWithdrawals).gt(MIN_DISPLAY_YOCTO) ? availableWithdrawals : '0', this.wallet),
      validators,
    }
  }
  ...
  async getValidatorInfo (address, validator) {
    const [total, staked, unstaking, availableForWithdraw] = await Promise.all([
      this.viewFunction(validator, 'get_account_total_balance', { account_id: address }),
      this.viewFunction(validator, 'get_account_staked_balance', { account_id: address }),
      this.viewFunction(validator, 'get_account_unstaked_balance', { account_id: address }),
      this.viewFunction(validator, 'is_account_unstaked_balance_available', { account_id: address }),
    ])
