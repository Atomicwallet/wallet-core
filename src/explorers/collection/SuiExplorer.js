...
export default class SuiExplorer extends Explorer {
  ...
  async getTransactions ({ address, offset, limit, pageNum }) {
    ...
    const response = [...balanceChangesFrom.filter(otherSideIsNotMe), ...balanceChangesTo.filter(otherSideIsNotMeAndAmountIsPositive)]
  ...
  async calculateFee (tx) {
    const dryRunResult = await this.provider.dryRunTransactionBlock({ transactionBlock: tx.transactionBlockBytes })
  ...
  async send (tx) {
    return this.provider.executeTransactionBlock({ transactionBlock: tx.transactionBlockBytes, signature: tx.signature })
  }
}
