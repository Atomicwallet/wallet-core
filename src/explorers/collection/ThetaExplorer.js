...
export default class ThetaExplorer extends Explorer {
  ...
  getTransactionsModifiedResponse (tx, selfAddress, currentBlock) {
    ...
    const matchedOutput = tx.data.outputs?.find((output) => output.address === lcSelfAddress) ||
      tx.data.outputs?.[0]
    const matchedInput = tx.data.inputs?.find((input) => input.address === lcSelfAddress) ||
      tx.data.inputs?.[0]

    if (!matchedOutput?.coins || !matchedInput ||
    ...
    return Object.entries(matchedOutput.coins).reduce((txs, [coin, amount]) => {
      ...
      if (new this.wallet.BN(amount).lte(
  ...
  getTxFee (tx) {
    return this.wallet.toCurrencyUnit(
      Object.values(tx.data.fee)
        .find((amount) => new this.wallet.BN(amount)
          .gt(new this.wallet.BN(0))
        )
    )
  }
  ...
  getTxFeeTicker (tx) {
    ...
    const matchedKey = Object.keys(fee)
      .find((key) => new this.wallet.BN(fee[key])
        .gt(new this.wallet.BN(0))
      )
