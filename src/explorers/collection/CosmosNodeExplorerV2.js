...
class CosmosNodeExplorerV2 extends CosmosNodeTransactionTypeMixin(Explorer) {
  ...
  getSendTransactionParams (rawtx) {
    return { tx_bytes: Buffer.from(rawtx).toString('base64'), mode: 'BROADCAST_MODE_SYNC' }
  }
  ...
  async getTransactions ({ address, limit, denom }) {
    const sent = await this
      .request(`/cosmos/tx/v1beta1/txs?events=message.sender=%27${address}%27&query=message.sender=%27${address}%27`)

    const recieved = await this
      .request(`/cosmos/tx/v1beta1/txs?events=transfer.recipient=%27${address}%27&query=transfer.recipient=%27${address}%27`)
    ...
    const txMessages = txResponses
      .flatMap((tx) => [...tx.tx.body.messages.map((msg) => {
        let fee
  ...
  async getGasEstimation (rawtx) {
    const { gas_info: { gas_used: gasUsed } } = await super.request(
