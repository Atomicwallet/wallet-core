...
class InsightExplorer extends Explorer {
  ...
  getAllowedTickers () {
    return [
      'BTC', 'LTC', 'ZEC', 'DGB', 'DASH', 'DOGE', 'BTG', 'QTUM', 'BCH',
      'BSV', 'SMART', 'DCR', 'GRS', 'YEC', 'RVN', 'KMD', 'BCD',
    ]
  }
  ...
  modifyUnspentOutputsResponse (selfAddress, response) {
    ...
    if (this.wallet.ticker === 'YEC') {
      return response
        .map(({ address, mintTxid: txid, mintIndex: vout, script, value }) => ({
          txid,
          vout,
          script,
          value,
  ...
  async sendTransaction (rawtx) {
    ...
    try {
      ...
      throw new ExplorerRequestError({ type: SEND_TRANSACTION_TYPE, error, url: urlToSend, instance: this })
    }
