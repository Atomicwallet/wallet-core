...
export default class RewardCalculator {
  ...
  async getRewards(ssnaddr, delegator) {
    const last_withdraw_cycle_map_response = await axios.post(
      this.baseUrl,
  ...
  async getRewardNeedCycleList(
    ...
    const reward_cycle_list_reverse = await this.getRewardCycleList(
      last_withdraw_cycle_map,
      last_reward_cycle_json,
      ssnaddr,
      delegator,
  ...
  async combineBuffDirect(ssnaddr, delegator, reward_list) {
    ...
    const deleg_stake_per_cycle_json_response = await axios.post(
      this.baseUrl,
      this.jsonBody([KEY_DELEG_PER_CYCLE, [delegator.toLowerCase(), ssnaddr]]),
    );
  ...
  async calculateRewards(ssnaddr, delegate_per_cycle, need_list) {
    ...
    const stake_ssn_per_cycle_map_response = await axios.post(
      this.baseUrl,
