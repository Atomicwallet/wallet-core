...
export default class AdaLibApi {
  constructor(cardanoWalletV2, cardanoWalletV4, feeParams) {
    ...
    this.protocolParams = {
      linearFee: this.cardano.LinearFee.new(
        this.cardano.BigNum.from_str(String(feeParams.feeCoefficient)),
        this.cardano.BigNum.from_str(String(feeParams.feeConst)),
      ),
  ...
  async getAddressByPrivateKey(cip = undefined) {
    ...
    const baseAddr = this.cardano.BaseAddress.new(
      ...
      this.cardano.StakeCredential.from_keyhash(utxoPubKey.to_raw_key().hash()),
      this.cardano.StakeCredential.from_keyhash(stakeKey.to_raw_key().hash()),
    );
  ...
  newAdaUnsignedTx(
    ...
    const unsignedTxResponse = this.newAdaUnsignedTxFromUtxo(
      receiver,
      amount,
      changeAdaAddr,
  ...
  addUtxoInput(txBuilder, input) {
    ...
    if (keyHash === null) {
      ...
      txBuilder.add_bootstrap_input(
        byronAddr,
        this.utxoToTxInput(input),
        this.cardano.Value.new(this.cardano.BigNum.from_str(input.amount)),
      );
  ...
  utxoToTxInput(utxo) {
    return this.cardano.TransactionInput.new(
      this.cardano.TransactionHash.from_bytes(Buffer.from(utxo.tx_hash, 'hex')),
      utxo.tx_index,
    );
  }
  ...
  signTransaction(
    ...
    const txBody =
      req.signRequest.unsignedTx instanceof this.cardano.TransactionBuilder
  ...
  addWitnesses({ txBody, uniqueUtxo, keys }) {
    ...
    for (let index = 0; index < uniqueUtxo.length; index += 1) {
      ...
      if (byronAddr) {
        const bootstrapWit = this.cardano.make_daedalus_bootstrap_witness(
          txHash,
          byronAddr,
          this.cardano.LegacyDaedalusPrivateKey.from_bytes(byronKey), // optional only for Byron bootstrap
        );
  ...
  estimateFee({ address, amount, outputs, ttl, certs }) {
    ...
    txBuilder.add_output(
      this.cardano.TransactionOutput.new(
        wasmReceiver,
        this.cardano.Value.new(this.cardano.BigNum.from_str(String(amount))),
      ),
    );
  ...
  createDelegationTransaction({
    ...
    const signRequest = this.newAdaUnsignedTx(
      ...
      null /* amount is null - send all */,
      paymentAddress /* as changeAddress */,
      utxo,
      slotNo,
      certs,
    );
