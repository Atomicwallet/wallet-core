...
export const createWallets = async (arg?: { id: string }, db?: IDataBase, configManager?: IConfigManager) => {
  ...
  return config.reduce((accum: Coin[], coinData: any) => {
    ...
    if (coins[coinClass]) {
      const wallet = createCoin(coins[coinClass], coinData, db, configManager);
