...
import { NftMixin } from '../../coins/nfts/mixins/index.js';
import { ExternalError } from '../../errors/index.js';
import ETHNftExplorer from '../../explorers/collection/ETHNftExplorer.js';
import FtmExplorer from '../../explorers/collection/FtmExplorer.js';
...
class FTMCoin extends Web3Mixin(NftMixin(HasProviders(HasTokensMixin(Coin)))) {
     ...
     */
    async createTransaction({ address, isSendAll = false, userFee = null, amount, nonce, userGasPrice, multiplier = this.gasPriceCoefficient, gasLimit, paymentData = null, }) {
    ...
    async checkTransaction(txInfo) {
        const { wallet, address, amount, memo, txid, nonce, fee, feeTicker } = txInfo;
        try {
            const newTx = new Transaction({
                ...
                confirmations: 1,
                fee,
                feeTicker: feeTicker ?? wallet.feeTicker ?? this.ticker,
            });
    ...
    async getNftFee({ contractAddress, tokenId, tokenStandard, toAddress = null, userOptions = {} }) {
        const targetAddress = !toAddress || toAddress.toLowerCase() === this.address.toLowerCase() ? MOCKED_FTM_ADDRESS : toAddress;
        try {
            const data = await this.getProvider('nft-send').getNftContractData(this, targetAddress, contractAddress, tokenId, tokenStandard);
    ...
    async estimateGas(amount, address, contract, defaultGas = DEFAULT_MAX_GAS) {
        const tokenSendData = this.getProvider('send').createSendTokenContract(contract, this.address, MOCKED_FTM_ADDRESS, amount);
        const coreLibrary = await this.getCoreLibrary();
    ...
    async getStakingInfo() {
        ...
        const tokens = Object.values(this.tokens).filter(({ contract }) => stakingContracts.some(({ address }) => address.toLowerCase() === contract.toLowerCase()));
        await this.getProvider('stake').getTokensInfo(tokens, this.address);
        const balances = tokens.reduce((acc, { contract, balance = '0', ticker, id, decimal }) => {
            /*
...
//# sourceMappingURL=FTMCoin.js.map