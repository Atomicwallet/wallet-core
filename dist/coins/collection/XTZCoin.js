...
import { Amount, LazyLoadedLib, LOAD_WALLET_ERROR, SEND_TRANSACTION_TYPE } from '../../utils/index.js';
...
class XTZCoin extends StakingMixin(HasProviders(Coin)) {
    ...
    async getAddress() {
        if (this.#privateKey) {
            const libsodiumWrappers = await this.getLibsodiumWrappers();
            return this.bs58EncodeWithPrefix(libsodiumWrappers.crypto_generichash(HASH_LENGTH, await this.bs58Decode(this.#privateKey, this.prefix.edsk).slice(PK_HASH_LENGTH)), this.prefix.tz1);
        }
    ...
    async create(operation) {
        ...
        const [fee, publicKey] = await Promise.all([
            this.getFee(),
            this.bs58EncodeWithPrefix((await this.bs58Decode(this.#privateKey, this.prefix.edsk)).slice(PK_HASH_LENGTH), this.prefix.edpk),
        ]);
        const config = {
            headers: {
        ...
        const resultRawTx = await axios.post(`${nodeUrl}/chains/${headerInfo.chain_id}/blocks/${headerInfo.hash}/helpers/forge/operations`, operationsObject, config);
        ...
        // check transaction
        const operationObjectResults = await axios.post(`${nodeUrl}/chains/main/blocks/head/helpers/preapply/operations`, [operationsObject], config);
        // check result
        ...
        const errors = operationObjectResults.data.reduce((acc, operationObjectResult, index) => {
            acc.push(...operationObjectResult.contents.reduce((opAcc, { destination, metadata }) => {
    ...
    async sign(rawTx) {
        ...
        const libsodiumWrappers = await this.getLibsodiumWrappers();
        const sig = libsodiumWrappers.crypto_sign_detached(libsodiumWrappers.crypto_generichash(PK_HASH_LENGTH, byteBuffer), await this.bs58Decode(this.#privateKey, this.prefix.edsk), 'uint8array');
...
//# sourceMappingURL=XTZCoin.js.map