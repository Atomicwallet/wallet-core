...
import { LazyLoadedLib, EXTERNAL_ERROR } from '../../utils/index.js';
...
class APTCoin extends HasProviders(Coin) {
    ...
    async validateAddress(address) {
        try {
            const { TxnBuilderTypes: { AccountAddress }, } = await this.loadLib();
    ...
    async createTransaction({ address, amount, userGasPrice, gasLimit: userGasLimit }) {
        const { AptosClient, BCS, TxnBuilderTypes: { AccountAddress, ChainId, EntryFunction, RawTransaction, TransactionPayloadEntryFunction }, } = await this.loadLib();
        const entryFunctionPayload = new TransactionPayloadEntryFunction(EntryFunction.natural(APTOS_TRANSFER_MODULE_NAME, APTOS_TRANSFER_FUNCTION_NAME, [], [BCS.bcsToBytes(AccountAddress.fromHex(address)), BCS.bcsSerializeUint64(BigInt(amount))]));
        try {
            ...
            const rawTxn = new RawTransaction(AccountAddress.fromHex(this.address), BigInt(sequenceNumber), entryFunctionPayload, 
            ...
            this.getTransactionExpirationTimeout(), new ChainId(APTOS_MAINNET_CHAIN_ID));
...
//# sourceMappingURL=APTCoin.js.map