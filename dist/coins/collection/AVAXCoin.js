...
class AVAXCoin extends Web3Mixin(NftMixin(HasProviders(HasBlockScanner(Coin)))) {
    ...
    async getFee({ userGasPrice = null, gasLimit = null } = {}) {
        return new this.BN(new this.BN(userGasPrice || (await this.getGasPrice()))).mul(new this.BN(gasLimit !== null ? gasLimit : this.gasLimit));
    }
    async getGasPrice(withoutCoeff = false) {
        try {
            ...
            const { fastest } = (await this.configManager?.get(ConfigKey.AvaxCGasPrice)) ?? (await this.getProvider('gas_price').getGasPrice());
...
//# sourceMappingURL=AVAXCoin.js.map