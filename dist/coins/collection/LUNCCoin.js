...
import TerraClassicFCDExplorer from '../../explorers/collection/TerraClassicFCDExplorer.js';
import TerraClassicLCDExplorer from '../../explorers/collection/TerraClassicLCDExplorer.js';
import TerraMantleExplorer from '../../explorers/collection/TerraMantleExplorer.js';
...
class LUNCCoin extends StakingMixin(HasProviders(HasTokensMixin(Coin))) {
    ...
    isTokenExcluded(token) {
        ...
        return !this.getExcludedTokenList().some(({ denom, contract }) => (parsedDenom && parsedDenom === denom) || (parsedContract && parsedContract === contract));
    }
    /**
     * Loads a wallet.
     *
    ...
    async availableBalance(fees) {
        return this.BigNumber(this.balance).isGreaterThan(0)
            ? this.toCurrencyUnit(this.BigNumber(this.balance)
                .minus(fees || 0)
                .toFixed())
            : '0';
    }
    ...
    async getUserTokenList(...args) {
        try {
            const [tokens, denoms] = await Promise.all([
                this.getProvider('token').getUserTokenList(this.address, true, // isClassic
                ...args),
    ...
    async getInfo(tokenInfo) {
        ...
        if (!tokenInfo || (!tokenInfo.onlyCoin && tokenInfo.isToken !== false)) {
            ...
            const tokensInfo = tokenInfo
                ...
                : this.tokens;
            await Promise.all(Object.values(tokensInfo).map(async (info) => {
                try {
                    let tokenBalance;
                    if (info.contract) {
                    ...
                    if (!tokenBalance) {
                        throw new Error(`${this.ticker} can't get balance of token with ${info.contract ? 'contract address' : 'denom'}
    ...
    async calculateAvailableForStake({ balance }) {
        const fee = ((Number(this.gasLimit?.[LUNC_SEND_TYPES.STAKE]) || FALLBACK_GASLIMIT[LUNC_SEND_TYPES.STAKE]) *
            (Number(this.gasPrices?.uluna) || Number(FALLBACK_GASPRICE.uluna))).toFixed(0);
...
//# sourceMappingURL=LUNCCoin.js.map