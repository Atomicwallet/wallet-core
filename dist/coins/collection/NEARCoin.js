...
class NEARCoin extends StakingMixin(HasBlockScanner(HasProviders(Coin))) {
    ...
    validateAddress(address) {
        return (address.length >= ADDRESS_MIN_LENGTH && address.length <= ADDRESS_MAX_LENGTH && ADDRESS_PATTERN.test(address));
    }
    /**
     * Create stake tx
     *
    ...
    async createDelegationTransaction({ amount, validator, nonce }) {
        ...
        const tx = nearAPI.transactions.createTransaction(this.address, publicKey, validator, accessKey.nonce + 1, actions, recentBlockHash);
    ...
    async createTransaction({ address, amount, nonce }) {
        ...
        const tx = nearAPI.transactions.createTransaction(this.address, publicKey, address, nonce || accessKey.nonce + 1, actions, recentBlockHash);
        return this.serializeAndSignTransaction(tx);
    }
    /**
    ...
    async getFee({ userGasPrice = null, gasLimit = null, contract, address } = {}) {
        return new this.BN(gasLimit || (await this.estimateGas(null, address, contract))).mul(new this.BN(userGasPrice || (await this.getGasPrice())));
    }
    ...
    async estimateGas(amount, address, contract) {
        ...
        return actions
            .reduce((summ, action) => summ.add(feeTypes.reduce((actionSumm, feeType) => actionSumm.add(new this.BN(action[feeType])), new this.BN(0))), new this.BN(0))
...
//# sourceMappingURL=NEARCoin.js.map