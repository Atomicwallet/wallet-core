...
import { DEFAULT_BINANCE_NET_URL } from '../../env.js';
import { WalletError } from '../../errors/index.js';
import BinanceBCExplorer from '../../explorers/collection/BinanceBCExplorer.js';
import BinanceDex from '../../explorers/collection/BinanceDex.js';
import BinanceExplorer from '../../explorers/collection/BinanceExplorer.js';
...
class BNBCoin extends StakingMixin(HasProviders(HasTokensMixin(Coin))) {
    ...
    constructor({ alias, notify, feeData, explorers, txWebUrl, socket, id }, db, configManager) {
        ...
        /** @type {BncClient|null} */
        this.bncClient = null;
        this.isBncClientInitialized = false;
    ...
    async initBncClient(bncClient) {
        ...
        try {
            ...
            console.warn('BNB coreLibrary load FAILED. It would be attempt to re-initialize during each sendTransaction call. ' +
                'Error thrown:', error);
        }
    }
    /**
    ...
    async placeOrder(quantity, price, side = 1, address = this.address, pairSymbol = 'AWC-986_BNB', sequence = null, timeInforce = DEFAULT_TIMEINFORCE) {
        const bncClient = await this.getInitializedBncClient();
    ...
    async calculateAvailableForStake() {
        const available = new this.BN(this.toMinimalUnit(await this.availableBalance())).sub(new this.BN(this.reserveForStake));
        return new Amount(available.isNeg() ? '0' : available, this);
    }
}
export default BNBCoin;
//# sourceMappingURL=BNBCoin.js.map