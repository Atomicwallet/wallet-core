...
import PolkadotNodeExplorer from '../../explorers/collection/PolkadotNodeExplorer.js';
...
class DOTCoin extends HasProviders(HasBlockScanner(Coin)) {
    ...
    async validateAddress(address) {
        const { Sr25519Account } = await UniqueNftLib.get();
        try {
            const encoded = await Sr25519Account.utils.encodeSubstrateAddress(Sr25519Account.utils.decodeSubstrateAddress(address), POLKADOT_ADDRESS_TYPE);
    ...
    async getInfo() {
        const { balance = undefined, balances = {}, nonce = undefined, } = await this.getProvider('balance').getInfo(this.address);
        if (balance) {
    ...
    async createTransaction({ address, amount }) {
        ...
        const [{ number, hash }, { genesisHash, specVersion, txVersion, specName, chainName }, metadata, { EXTRINSIC_VERSION }, { getRegistry, construct, methods: substrateMethods },] = await Promise.all([
            this.getLatestBlock(),
            this.getTxMeta(),
            this.getMetadata(),
...
//# sourceMappingURL=DOTCoin.js.map