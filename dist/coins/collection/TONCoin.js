...
class TONCoin extends HasProviders(HasTokensMixin(Coin)) {
    ...
    async getFee({ address, sendAmount, custom = '' } = {}) {
        ...
        const result = new this.BN(Object.values(estimation.source_fees).reduce((prev, cur) => (cur !== 'fees' ? prev + cur : prev), feeReserve));
        return result;
    }
    ...
    async getInfo(props) {
        ...
        const { state, balance } = (await this.getProvider(BALANCE_PROVIDER_OPERATION).getState(this.address).catch()) ?? {};
        if (balance) {
...
//# sourceMappingURL=TONCoin.js.map