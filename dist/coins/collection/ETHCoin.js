...
import { ETHNftExplorer, MoralisExplorer, Web3Explorer } from '../../explorers/collection/index.js';
...
class ETHCoin extends StakingMixin(Web3Mixin(NftMixin(HasProviders(HasTokensMixin(Coin))))) {
    ...
    async getTransactions() {
        ...
        const { transactions = [], tokenTransactions = [], failed = [], } = await this.getProvider('history').getTransactions({
            address: this.address,
        });
        const failedTxs = await Promise.all(failed.map(({ txid }) => this.getProvider('history').getTransaction(this.address, txid, this.tokens)));
...
//# sourceMappingURL=ETHCoin.js.map