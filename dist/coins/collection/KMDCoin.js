...
class KMDCoin extends HasProviders(BitgoMixin(BitcoinLikeFeeMixin(Coin))) {
    ...
    constructor({ alias, notify, feeData, explorers, txWebUrl, socket, id }) {
        ...
        const config = {
            ...
            socket,
            dependencies: {
                [BITGO]: new LazyLoadedLib(() => import('bitgo-utxo-lib-legacy').then(({ default: bitgoLib }) => {
    ...
    async createClaimTransaction(rewardObject) {
        ...
        const tx = await this.buildTx(inputs, this.address, balance.add(reward).toNumber(), new this.BN(0), this.#privateKey);
        return tx;
    }
    async getInfo() {
...
//# sourceMappingURL=KMDCoin.js.map