...
class ARBCoin extends Web3Mixin(NftMixin(HasProviders(HasTokensMixin(Coin)))) {
    ...
    async getNftFee({ contractAddress, tokenId, tokenStandard, toAddress = null, userOptions = {} }) {
        const targetAddress = !toAddress || toAddress.toLowerCase() === this.address.toLowerCase() ? MOCKED_ARB_ADDRESS : toAddress;
        try {
    ...
    async getUserTokenList() {
        const userTokens = ((await this.getProvider('token')
            ...
            .catch((error) => console.warn(error))) || []).filter((token) => token.supportedStandards?.includes('erc20'));
...
//# sourceMappingURL=ARBCoin.js.map