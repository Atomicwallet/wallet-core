...
class ADACoin extends HasProviders(Coin) {
    ...
    constructor({ alias, notify, feeData, explorers, txWebUrl, submitUrl, socket, id }, db, configManager) {
        ...
        this.loadLibsPromise = preventConcurrent(this.loadLibs.bind(this));
    ...
    async loadLibs() {
        ...
        if (isNode) {
            ...
            modules = await Promise.all(isIOS
    ...
    async stake(poolId) {
        ...
        const { [stakeAddressHex]: regHistory } = await this.getProvider('regHistory').getRegistrationHistory(stakeAddressHex);
...
//# sourceMappingURL=ADACoin.js.map