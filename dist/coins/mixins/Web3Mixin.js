...
import * as ABICollection from '../../tokens/ABI/index.js';
import ERC20Default from '../../tokens/ABI/ERC-20/standard.js';
...
const Web3Mixin = (superclass) => class extends superclass {
    /**
    ...
    createSmartContractCall({ type = 'ERC20', smartContractAddress, standard = false, action, args = [] } = {}) {
        ...
        const supportedSmartContract = ABICollection[type.toUpperCase()]?.[standard ? 'standard' : smartContractAddress.toLowerCase()];
        ...
        if (!name) {
            throw new Error(`Smart-contract action '${action}' is not supported, supported actions:
        ...
        if (typeof contractInterface.methods[name] !== 'function') {
            throw new TypeError(`${this.constructor.name}: "${name}" is not implemented in provided smart-contract ABI methods`);
        }
    ...
    async getContractConstants(contract) {
        ...
        const constants = ERC20Default.filter(({ constant, type, inputs }) => constant && type === 'function' && inputs.length === 0);
        const info = {};
        const promises = [];
...
//# sourceMappingURL=Web3Mixin.js.map