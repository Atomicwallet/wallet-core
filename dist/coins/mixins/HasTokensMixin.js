...
const HasTokensMixin = (superclass) => class extends superclass {
    ...
    async loadTokensList(wallets) {
        try {
            ...
            const processedTokens = this.processTokenList(await this.setTokensConfirmation(remoteTokens, wallets, source), source);
    ...
    async fetchUserTokens(wallets) {
        ...
        const processedTokens = await this.processTokenList(await this.setTokensConfirmation(userTokens, wallets, TOKEN_SOURCE_USER), TOKEN_SOURCE_USER);
    ...
    async setTokensConfirmation(tokens = [], wallets, source) {
        ...
        return tokens.reduce((acc, next) => {
            ...
            const [matchingTokenInConfirmedTokens] = confirmedTokens.filter(({ contract, owner }) => (contract?.toLowerCase() || owner?.toLowerCase()) ===
    ...
    processTokenList(serverTokenList = [], source) {
        ...
        return serverTokenList
            ...
            .filter((token) => token.ticker.trim() !== '' &&
    ...
    async createCustomToken({ name, ticker, decimal, contract, uniqueField, source = 'custom' }, wallets) {
        ...
        const tokenArgs = await this.setTokensConfirmation([{ uniqueField, visibility: true, source, isCustom: true, ...args }], wallets);
    ...
    createTokens(tokens = [], wallets) {
      ...
      */
        ...
        tokensToCreate.forEach((token) => {
            const existingToken = wallets.getWallet(getTokenId({
    ...
    async getUniquesAndDuplicates(tokens) {
        ...
        const validPairs = new Set(tokens
...
//# sourceMappingURL=HasTokensMixin.js.map