...
export default class Explorer {
    ...
    async getInfo(address) {
        ...
        const response = await this.request(this.getInfoUrl(address), this.getInfoMethod(), this.getInfoParams(address), GET_BALANCE_TYPE, this.getInfoOptions());
    ...
    async getTransactions({ address, offset, limit, pageNum }) {
        ...
        const response = await this.request(this.getTransactionsUrl(address, offset || 0, limit || this.defaultTxLimit, pageNum), this.getTransactionsMethod(), this.getTransactionsParams(address, offset || 0, limit || this.defaultTxLimit, pageNum), GET_TRANSACTIONS_TYPE, this.getTransactionsOptions());
    ...
    async getUnspentOutputs(address, scriptPubKey) {
        const response = await this.request(this.getUnspentOutputsUrl(address), this.getUnspentOutputsMethod(), this.getUnspentOutputsParams(address), GET_UTXO_TYPE, this.getUtxoOptions());
    ...
    async sendTransaction(rawtx) {
        const response = await this.request(this.getSendTransactionUrl(), this.getSendTransactionMethod(), this.getSendTransactionParams(rawtx), SEND_TRANSACTION_TYPE, this.getSendOptions());
    ...
    handleRequestError(error, { url, method, params, type, options }) {
        const { headers: requestHeaders, method: requestMethod, baseURL, url: requestUrl, data: requestData, } = error?.config || {};
        const errorData = {
            request: {
    ...
    async request(url, method = 'get', params = {}, type = UNDEFINED_OPERATION_ERROR, options = {}) {
        ...
        try {
            return this.modifyGeneralResponse(await this.client.request({
                url,
                method,
    ...
    async getLatestBlock() {
        const response = await this.request(this.getLatestBlockUrl(), this.getLatestBlockMethod(), this.getLatestBlockParams(), GET_LATEST_BLOCK_TYPE, this.getLatestBlockOptions());
    ...
    async getBlock(hash) {
        const response = await this.request(this.getBlockUrl(hash), this.getBlockMethod(), this.getBlockParams(), GET_BLOCK_TYPE, this.getBlockOptions());
