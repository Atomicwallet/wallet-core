...
class TzindexExplorer extends Explorer {
    ...
    async getTransaction(selfAddress, txid) {
        const response = await this.request(this.getTransactionUrl(txid), this.getTransactionsMethod(), this.getTransactionsParams(selfAddress, 0, 0), 'tx', this.getTransactionsOptions());
        return this.modifyTransactionResponse(response, selfAddress);
    }
    async getTransactions({ address }) {
        const limit = 100;
        const MAX_TRIES = 5;
        const meta = await this.request(this.getTransactionsUrl(address), this.getTransactionsMethod(), this.getTransactionsParams(address, 0, 0), 'txs', this.getTransactionsOptions());
        ...
        let tries = 0;
        do {
            const response = await this.request(this.getTransactionsUrl(address), this.getTransactionsMethod(), this.getTransactionsParams(address, offset, limit), 'txs', this.getTransactionsOptions());
            tries++;
...
//# sourceMappingURL=TzindexExplorer.js.map