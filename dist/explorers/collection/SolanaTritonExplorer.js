...
import { Connection, Keypair, PublicKey, StakeProgram, Transaction as SolanaTransaction } from '@solana/web3.js';
...
const getIsCoinTx = (tx) => {
    const { transaction: { message: { instructions: [{ program, parsed: { type } = {} }] = [{}] } } = {} } = tx || {};
    return program === SYSTEM_PROGRAM && type === TRANSFER_TYPE;
};
/** @type {DetectFunction} */
const getIsNftTx = (tx) => {
    const { meta: { postTokenBalances = [] } = {} } = tx || {};
    return postTokenBalances.length > 0 && postTokenBalances[0]?.uiTokenAmount?.decimals === 0;
};
/** @type {DetectFunction} */
...
const parseCoinTx = (tx) => {
    const { transaction: { message: { instructions: [{ parsed: { info: { source, destination, lamports: amount = 0 }, }, },], }, }, } = tx;
...
const parseNftTx = (tx) => {
    ...
    return postTokenBalances.reduce((instruction, tokenBalance) => {
        const { owner, uiTokenAmount: { amount }, } = tokenBalance;
        switch (amount) {
            case '1':
...
const parseTokenTx = (tx, tokensFromDb) => {
    const transferInstruction = tx.transaction?.message?.instructions.find((instruction) => {
        return instruction.program === 'spl-token' && instruction.parsed.type === 'transferChecked';
    });
    const info = transferInstruction.parsed.info;
    if (!info) {
        throw new Error('Could not parse token tx');
    }
    const { tokenAmount: { amount, decimals: decimal } = {}, destination = '', source, mint, } = info ?? { tokenAmount: {} };
...
class SolanaTritonExplorer extends Explorer {
     ...
     * @returns {Promise<string[]>}
     */
    async fetchTxSignaturesForChunk(pubKey, limit = ATOMIC_HISTORY_SIGNATURES_CHUNK_SIZE) {
        try {
            return (await this.connection.getSignaturesForAddress(pubKey, { limit }, FINALIZED_COMMITMENT)).map(({ signature }) => signature);
        }
        catch (error) {
            console.warn(error);
            return [];
    ...
    getTxInstruction(tx, tokensFromDb) {
        ...
        return Object.entries(TX_INSTRUCTION_PARSERS).reduce((instruction, [, { isMatch, parse }]) => {
    ...
    async getTransactions({ address, limit = this.defaultTxLimit, pageNum = 0 }) {
        const signatures = await this.fetchTxSignaturesForChunk(new PublicKey(address), limit * (pageNum + 1));
        const txs = await this.fetchTxsForChunk(chunkArray(signatures, limit)[pageNum] || []);
    ...
    modifyTransactionsResponse(txs, selfAddress) {
        const tokensFromDb = Object.fromEntries(Object.entries(this.wallet.tokens()));
        return txs.reduce((list, tx, index) => {
            try {
                const { source, destination, isNft, isToken, amount: txAmount, decimal, symbol, mint, } = this.getTxInstruction(tx, tokensFromDb);
                let amount;
                let walletid;
                if (isNft) {
                    amount = 'NFT';
    ...
    async sendNft(coin, toAddress, contractAddress, tokenId, tokenStandard, options) {
        ...
        try {
            ...
            const transaction = new SolanaTransaction().add(createTransferCheckedInstruction(fromTokenAccount.address, mintPublicKey, toTokenAccount.address, fromKeypair.publicKey, 1, 0, [], TOKEN_PROGRAM_ID));
    ...
    async sendTokenTransaction(coin, mint, toAddress, amount, decimals) {
        ...
        try {
            const fromTokenAccount = await getOrCreateAssociatedTokenAccount(this.connection, fromKeypair, mintPublicKey, fromKeypair.publicKey);
            const toTokenAccount = await getOrCreateAssociatedTokenAccount(this.connection, fromKeypair, mintPublicKey, destPublicKey);
            const transaction = new SolanaTransaction().add(createTransferCheckedInstruction(fromTokenAccount.address, mintPublicKey, toTokenAccount.address, fromKeypair.publicKey, BigInt(amount), decimals, [], TOKEN_PROGRAM_ID));
            const txid = await this.connection.sendTransaction(transaction, [fromKeypair]);
            return { txid };
        }
        catch (error) {
            console.warn(error);
...
//# sourceMappingURL=SolanaTritonExplorer.js.map