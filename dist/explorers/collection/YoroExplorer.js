...
import { SEND_TRANSACTION_TYPE, GET_UTXO_TYPE, GET_BALANCE_TYPE } from '../../utils/const/index.js';
class YoroExplorer extends Explorer {
    ...
    async getTransactions({ address, offset = 0, limit = this.defaultTxLimit }) {
        ...
        return txs
            .filter(((tx) => ({ txid }) => !tx.has(txid) && tx.add(txid))(new Set()))
    ...
    async getTransactionsQueued({ blockHash, address, recursion = false, after = undefined }) {
        ...
        const response = await this.request(this.getTransactionsUrl(address), this.getTransactionsMethod(), this.getTransactionsParams(address, untilBlock, after), 'GetTxs', this.getInfoOptions()).catch(() => {
            this.updating = false;
        });
        if (!response) {
            this.updating = false;
    ...
    getTxValue(selfAddress, tx) {
        ...
        let value;
        if (sentToMyself) {
            const outputsAmount = tx.outputs.reduce((prev, cur) => prev.add(new this.wallet.BN(cur.amount)), new this.wallet.BN(0));
            const inputsAmount = tx.inputs.reduce((prev, cur) => prev.add(new this.wallet.BN(cur.amount)), new this.wallet.BN(0));
    ...
    async getUTXO(address) {
        const result = await this.request(this.getUTXOUrl(), 'post', { addresses: [address] }, undefined, this.getInfoOptions()).catch((error) => {
            throw new ExplorerRequestError({
                type: GET_UTXO_TYPE,
                error,
                url: this.getUTXOUrl(),
    ...
    async sendTransaction({ rawtx, txid }) {
        ...
        const response = await this.request(this.getBroadcastUrl(), 'post', {
            signedTx: base64tx,
        }, undefined, this.getInfoOptions()).catch((error) => {
            throw new ExplorerRequestError({
    ...
    async getRegistrationHistory(stakeAddressHex) {
        return this.request(this.getRegistrationHistoryUrl(), 'post', {
            addresses: [stakeAddressHex],
        }, undefined, this.getInfoOptions());
    }
    async getAccountState(stakeAddressHex) {
        return this.request(this.getAccountStateUrl(), 'post', {
            addresses: [stakeAddressHex],
        }, undefined, this.getInfoOptions());
    }
}
export default YoroExplorer;
//# sourceMappingURL=YoroExplorer.js.map