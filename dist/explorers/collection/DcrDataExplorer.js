...
class DcrDataExplorer extends Explorer {
    ...
    async getTransactions({ address, offset = 0, limit = this.defaultTxLimit }) {
        const response = await this.request(this.getTransactionsUrl(address, offset, limit), this.getTransactionsMethod(), this.getTransactionsParams(), 'GetTxs');
    ...
    getTxDirection(selfAddress, tx) {
        return (tx.vin &&
            !tx.vin.find(({ prevOut }) => prevOut.addresses && prevOut.addresses.length > 0 && prevOut.addresses[0] === selfAddress));
    }
    /**
...
//# sourceMappingURL=DcrDataExplorer.js.map