...
import { ERC1155_TOKEN_STANDARD, ERC721_TOKEN_STANDARD, erc1155StandardTest, erc721StandardTest, UNRECOGNIZED_TOKEN_STANDARD, } from '../../coins/nfts/ETHNftToken.js';
import { getTransformedTokenUri } from '../../coins/nfts/utils/index.js';
import { MORALIS_API_KEY, MORALIS_NATIVE_API } from '../../env.js';
...
class MoralisExplorer extends Explorer {
    ...
    async getInfo(address, isSpamNftsEnabled) {
        try {
            const response = await this.request(this.getInfoUrl(address), this.getInfoMethod(), this.getInfoParams(address, this.chain, isSpamNftsEnabled));
    ...
    async modifyInfoResponse(response) {
        ...
        const nftList = nftRawList.map((rawNft, index) => {
            const { token_address: contractAddress, token_id: tokenId, token_uri: tokenUri, contract_type: tokenStandard, metadata, } = rawNft;
            if (!metadata) {
                ...
                metadataPromises.push(Promise.all([
                    index,
    ...
    async fetchNftList(coin, isSpamNftsEnabled) {
        ...
        return rawList.map(({ contractAddress, tokenId, tokenStandard, name, description, imageUrl }) => new ETHNftToken(contractAddress, tokenId, coin.id, this.fixTokenStandard(tokenStandard), name, description, imageUrl));
    }
    /**
    ...
    async getRawTokenTransactions({ address, limit = this.defaultTxLimit, cursor = null }) {
        try {
            const response = await this.request(this.getTokenTransactionsUrl(address), AXIOS_GET_METHOD, this.getTokenTransactionsParams(limit, cursor), GET_TRANSACTIONS_TYPE, this.getTransactionsOptions());
    ...
    async getUserTokenList(address) {
        const results = await this.request(this.getUserTokenListUrl(address), AXIOS_GET_METHOD, this.getInfoParams(address, this.chain), '', this.getTransactionsOptions());
    ...
    async getNftTransactions({ address, limit = this.defaultTxLimit, cursor = null }) {
        try {
            const response = await this.request(this.getNftTransactionsUrl(address), AXIOS_GET_METHOD, this.getNftTransactionsParams(limit, cursor), GET_TRANSACTIONS_TYPE, this.getTransactionsOptions());
...
//# sourceMappingURL=MoralisExplorer.js.map