...
import { GET_UTXO_TYPE, SEND_TRANSACTION_TYPE } from '../../utils/const/index.js';
class CardanoliteExplorer extends Explorer {
    ...
    getTxValue(selfAddress, tx) {
        ...
        let value;
        if (sentToMyself) {
            const outputsAmount = tx.outputs_amount.reduce((prev, cur) => prev.add(new this.wallet.BN(cur)), new this.wallet.BN(0));
            const inputsAmount = tx.inputs_amount.reduce((prev, cur) => prev.add(new this.wallet.BN(cur)), new this.wallet.BN(0));
...
//# sourceMappingURL=CardanoliteExplorer.js.map