...
class BlockbookV2Explorer extends Explorer {
    ...
    async getTokenTransactions({ address, offset, limit, pageNum, contract }) {
        ...
        try {
            const response = await this.request(this.getTransactionsUrl(address, offset || 0, limit || this.defaultTxLimit, pageNum), this.getTransactionsMethod(), this.getTokenTransactionsParams(address, offset || 0, limit || this.defaultTxLimit, pageNum, contract), GET_TRANSACTIONS_TYPE, this.getTransactionsOptions());
    ...
    modifyTransactionResponse(response, address) {
        if (response.tokenTransfers && Number(response.value) === 0) {
            ...
            response.tokenTransfers.forEach((transfer) => {
                ...
                tokenTransactions[transfer.token].push(this.getTransactionFromTokenTransfer({
                    transfer,
                    selfAddress: address,
                    tx: response,
                }));
    ...
    modifyTransactionsResponse(response, selfAddress) {
        ...
        const transactions = condition
            ? response.transactions.filter((tx) => (!tx.tokenTransfers || Number(tx.value) !== 0) &&
    ...
    modifyTokenTransactionsResponse(response, selfAddress) {
        return (response
            ...
            return tx.tokenTransfers
                ...
                .map((transfer) => this.getTransactionFromTokenTransfer({
                transfer,
                selfAddress,
                tx,
            }));
    ...
    filterTokenTransferTransactions(selfAddress, transfer) {
        if (![transfer.to.toLowerCase(), transfer.from.toLowerCase()].includes(selfAddress.toLowerCase()) ||
            !transfer.symbol) {
            return false;
        }
        return true;
    ...
    getTransactionFromTokenTransfer({ transfer, selfAddress, tx }) {
        return new Transaction({
            ...
            otherSideAddress: transfer.to.toLowerCase() === selfAddress.toLowerCase()
    ...
    getTxDirection(selfAddress, tx) {
        ...
        const hasOut = tx.vout &&
            tx.vout
                .filter(({ addresses }) => Array.isArray(addresses))
                .some(({ addresses }) => addresses.some((address) => address === selfAddress));
        const hasOtherOut = tx.vout &&
            tx.vout
                .filter(({ addresses }) => Array.isArray(addresses))
                .some(({ addresses }) => addresses.some((address) => address !== selfAddress));
        if (hasOut && !hasIn) {
    ...
    getUserTokenList(address) {
        return this.request(this.getInfoUrl(address), 'get', this.getUserTokenListParams(), '', this.getTransactionsOptions()).then((data) => data.tokens.map((token) => ({
    ...
    async getTransactionUnmodified(txId) {
        return this.request(this.getTransactionUrl(txId), this.getTransactionMethod(), this.getTransactionParams(txId), GET_TRANSACTION_TYPE, this.getTransactionOptions());
    }
    getTxFee(tx) {
...
//# sourceMappingURL=BlockbookV2Explorer.js.map