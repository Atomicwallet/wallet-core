...
class NanonodeExplorer extends Explorer {
    ...
    async workGenerate(hash) {
        const response = await this.request(this.getBaseUrl(), this.getActionMethod(), this.workGenerateParams(hash), 'Work generate', this.getSendOptions());
    ...
    async getPendingTransactions(address) {
        const response = await this.request(this.getBaseUrl(), this.getActionMethod(), this.pendingTransactionsParams(address));
        return response;
    }
    /**
     * @param  {} hash - block hash
    ...
    async getBlockInfo(hash) {
        const response = await this.request(this.getBaseUrl(), this.getActionMethod(), this.blockInfoParams(hash), 'Block info', this.getSendOptions());
    ...
    async getTransactions({ address, offset = 0, limit = this.defaultTxLimit, pending }) {
        const response = await this.request(this.getBaseUrl(), this.getTransactionsMethod(), this.getTransactionsParams(address, offset, limit), GET_TRANSACTIONS_TYPE);
        if (response.error) {
            throw new Error(response.error);
        }
...
//# sourceMappingURL=NanonodeExplorer.js.map