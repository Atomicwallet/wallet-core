import { TONWEB_API_KEY, TONWEB_FALLBACK_INDEX_URL, TONWEB_FALLBACK_V2_URL } from '../../env.js';
...
export default class TonwebExplorer extends Explorer {
    ...
    static { this.ownJettonsWalletsAddressesToTokenUniqueFieldObj = null; }
    ...
    async _getOwnJettonsWalletsAddressesToTokenUniqueFieldObj() {
        ...
        const jettonsWalletsAddresses = await Promise.all(tokensList.map((token) => this.getJettonWalletAddress(this.wallet.address, token[1].mint)));
    ...
    async modifyTokensTransactionsResponse(txs) {
        ...
        return txs.reduce((list, tx, index) => {
            try {
                const { source, destination, isToken, amount: txAmount, decimal, symbol, name: tokenName, mint, } = this._getTokenTxInstruction(tx);
                const isValidBaseProperties = source && destination && txAmount;
    ...
    async getTokenTransactions({ jettonWalletAddress }) {
        ...
        const response = await this.request(this.getTransactionsUrl(), this.getTransactionsMethod(), this.getTransactionsParams(jettonWalletAddress), GET_TRANSACTIONS_TYPE, this.getTransactionsOptions());
        return this.modifyTokensTransactionsResponse(response, this.wallet.address);
    }
}
//# sourceMappingURL=TonwebExplorer.js.map