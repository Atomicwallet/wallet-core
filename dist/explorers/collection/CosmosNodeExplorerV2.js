...
class CosmosNodeExplorerV2 extends CosmosNodeTransactionTypeMixin(Explorer) {
    ...
    async getTransactions({ address, limit, denom }) {
        const sent = await this.request(`/cosmos/tx/v1beta1/txs?events=message.sender=%27${address}%27&query=message.sender=%27${address}%27`);
        const recieved = await this.request(`/cosmos/tx/v1beta1/txs?events=transfer.recipient=%27${address}%27&query=transfer.recipient=%27${address}%27`);
        const txMap = new Map();
        ...
        const txMessages = txResponses
            ...
            ...tx.tx.body.messages.map((msg) => {
                ...
                return {
                    ...msg,
                    ...pick(tx, 'from_address', 'to_address', 'amount', 'txhash', 'height', 'gas_used', 'gas_wanted', 'direction', 'logs', 'timestamp'),
    ...
    async getUnbondingDelegations(address) {
        try {
            const { unbonding_responses: delegations } = await this.request(`cosmos/staking/v1beta1/delegators/${address}/unbonding_delegations`);
            return delegations;
        }
        catch (error) {
            console.warn(error);
    ...
    async getGasEstimation(rawtx) {
        const { gas_info: { gas_used: gasUsed }, } = await super.request('cosmos/tx/v1beta1/simulate', 'post', this.getSendTransactionParams(rawtx));
        return gasUsed;
    }
}
export default CosmosNodeExplorerV2;
//# sourceMappingURL=CosmosNodeExplorerV2.js.map