...
class ZilliqaNodeExplorer extends ZilliqaAbstractExplorer {
    ...
    async sendTransaction({ rawtx, privateKey }) {
        ...
        try {
            ...
            response = await this.request(this.config.baseUrl, 'POST', {
                id: 'atomic',
                jsonrpc: '2.0',
    ...
    async getBalance(address, contract) {
        const { balance, nonce = 0 } = (await this.getInfo(this.toValidChecksumAddress(address)).catch((error) => console.warn(error))) || {};
        return {
            balance,
            nonce: Number(nonce),
        };
    ...
    async getTokenBalance(address, contracts) {
        const balances = contracts
            ...
            const contractSubState = await this.request(this.config.baseUrl, 'POST', {
                id: 'atomic',
                jsonrpc: '2.0',
    ...
    async getStakedAmountFromContract(address, ssnContract) {
        const legacyAddr = `0x${this.toValidChecksumAddress(address)}`;
        const subStateDepositResponse = await this.request(this.config.baseUrl, 'POST', {
            id: 'atomic',
            jsonrpc: '2.0',
            method: RPCMethod.GetSmartContractSubState,
            params: [ssnContract, 'deposit_amt_deleg', [legacyAddr]],
        }, GET_BALANCE_TYPE);
        const subStateBufferedResponse = await this.request(this.config.baseUrl, 'POST', {
            id: 'atomic',
            jsonrpc: '2.0',
            method: RPCMethod.GetSmartContractSubState,
        ...
        return Object.keys(deposit_amt_deleg[legacyAddr] || {}).reduce((acc, validator) => {
            acc.validators[validator] = {
                ...
                buffered: Object.keys(buff_deposit_deleg).length > 0 && buff_deposit_deleg[legacyAddr][validator]
    ...
    async getWithdrawalsFromContract(address, ssnContract) {
        ...
        const subState = await this.request(this.config.baseUrl, 'POST', {
            id: 'atomic',
            jsonrpc: '2.0',
            method: RPCMethod.GetSmartContractSubState,
    ...
    async getBlockchainInfo() {
        const { result } = await this.request(this.config.baseUrl, 'POST', {
            id: 'atomic',
            jsonrpc: '2.0',
...
//# sourceMappingURL=ZilliqaNodeExplorer.js.map