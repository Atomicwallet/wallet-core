...
export const createHash64 = (salt, length = 64) => {
    return createHash('sha256').update(salt).digest('hex').slice(0, length);
};
export const createCosmoMockSignedData = (address) => {
...
export const generateMockUtxo = (address, value) => {
    return [
        {
            address,
            outputIndex: 1,
...
export const mnemonicSeedsList = mnemonicPhrasesList.map((mnemonicPhrase) => new BitcoreMnemonic(mnemonicPhrase).toSeed());
export const DATA_MESSAGE = 'Signed data message';
export const DATA_STRING = `0x${Buffer.from(DATA_MESSAGE, 'utf8').toString('hex')}`;
export const mnemonicMappingKeys = mapping;
